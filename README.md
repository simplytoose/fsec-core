...
Проведено тестування платформи під навантаженням за допомогою **Gatling** для перевірки стійкості до оверселінгу (відсутність дублів/проблем з паралельними покупками).

### 📈 Графіки RPS та Latency
Затримка (Latency) та кількість запитів в секунду (RPS) під навантаженням:
![RPS and Latency](assets/gatling_rps_latency.png)

### ✅ Відсутність дублів (Idempotency Check)
100% успішних замовлень без дублікатів навіть під час високого конкурентного навантаження (Race conditions).
![No Duplicates](assets/gatling_no_duplicates.png)
